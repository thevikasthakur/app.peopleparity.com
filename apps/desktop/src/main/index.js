...
const productiveHoursService_1 = require("./services/productiveHoursService");
...
if (process.platform === 'darwin') {
    ...
    electron_1.app.whenReady().then(() => {
        ...
        if (scaleFactor === 2) {
            ...
            electron_1.app.commandLine.appendSwitch('use-angle', 'metal');
            electron_1.app.commandLine.appendSwitch('enable-features', 'MetalBackend');
        }
    });
...
if (electron_1.app.isPackaged) {
    const logFile = path_1.default.join(electron_1.app.getPath('userData'), 'main-process.log');
...
const getAppIcon = () => {
    const possiblePaths = [
        path_1.default.join(__dirname, '../../build/icon.ico'), // Development path
        path_1.default.join(__dirname, '../../../build/icon.ico'), // Production path
        path_1.default.join(process.resourcesPath, 'icon.ico'), // Packaged app resources
        path_1.default.join(electron_1.app.getAppPath(), 'build/icon.ico'), // App path
        path_1.default.join(electron_1.app.getAppPath(), 'icon.ico'), // Root of app
    ];
    ...
    const template = [
        ...
        },
        {
            ...
            submenu: [
                ...
                ...(process.env.NODE_ENV === 'development' || !electron_1.app.isPackaged ? [{ role: 'toggleDevTools' }] : []),
                { type: 'separator' },
...
const createTray = async () => {
    ...
    const localIconPath = path_1.default.join(__dirname, `../assets/tray-icon${iconExt}`);
    const srcIconPath = path_1.default.join(__dirname, `../../src/assets/tray-icon${iconExt}`);
    let iconPath = null;
    if (fs_1.default.existsSync(localIconPath)) {
        iconPath = localIconPath;
        console.log('✅ Found bundled icon at:', localIconPath);
    }
    else if (fs_1.default.existsSync(srcIconPath)) {
    ...
    if (!iconPath) {
        try {
            ...
            const tempIconPath = path_1.default.join(electron_1.app.getPath('userData'), `tray-icon${iconExt}`);
            // Check if we already have it downloaded
            if (fs_1.default.existsSync(tempIconPath)) {
                trayIcon = electron_1.nativeImage.createFromPath(tempIconPath);
                ...
                const file = fs_1.default.createWriteStream(tempIconPath);
                await new Promise((resolve, reject) => {
                    https.get(iconUrl, (response) => {
                        ...
                        fs_1.default.unlink(tempIconPath, () => { }); // Delete incomplete file
                        reject(err);
                    });
                });
                trayIcon = electron_1.nativeImage.createFromPath(tempIconPath);
    ...
    if (!trayIcon || trayIcon.isEmpty()) {
        ...
        trayIcon = electron_1.nativeImage.createFromBuffer(buffer, { width: size, height: size });
...
const updateTrayMenu = () => {
    ...
    if (currentSession) {
        ...
        currentNote = currentActivity?.currentNote || currentSession.task || 'No note set';
    ...
    const storedActivities = store.get('recentActivities') || [];
...
electron_1.app.whenReady().then(async () => {
    ...
    activityTracker.on('inactivity:detected', (message) => {
    ...
    productiveHoursService = new productiveHoursService_1.ProductiveHoursService(databaseService);
    permissionsService = new permissionsService_1.PermissionsService();
    ...
    try {
        ...
        if (activeSession) {
            ...
            if (latestScreenshot) {
                ...
                const screenshotDateUTC = new Date(Date.UTC(screenshotTime.getUTCFullYear(), screenshotTime.getUTCMonth(), screenshotTime.getUTCDate()));
    ...
    activityTracker.on('session:midnight-stop', (data) => {
    ...
    const sessionStartedListener = async (session) => {
    ...
    electron_1.app.on('concurrent-session-detected', async (event) => {
        ...
        dialog.showErrorBox('Concurrent Session Detected', `Time tracking is already active on ${otherDevice}. This session has been stopped to prevent duplicate time tracking.`);
...
function setupIpcHandlers() {
    ...
    electron_1.ipcMain.handle('auth:get-api-url', async () => {
    ...
    electron_1.ipcMain.handle('session:update-note', async (_, note) => {
        const activeSession = databaseService.getActiveSession();
        if (!activeSession) {
    ...
    electron_1.ipcMain.on('note-updated', () => {
        updateTrayMenu();
    });
    // Sync recent activities from renderer's localStorage
    electron_1.ipcMain.handle('sync-recent-activities', async (_, activities) => {
    ...
    electron_1.ipcMain.handle('session:today', async (_event, dateString) => {
        if (dateString) {
    ...
    electron_1.ipcMain.handle('session:productive-info', async () => {
        ...
        const db = databaseService.localDb;
    ...
    `).all(session.id);
      ...
      `).all(screenshot.id);
            ...
            screenshotDetails.push({
                ...
                classification: uiScore >= 8.5 ? 'good' :
                    uiScore >= 7.0 ? 'fair' :
                        uiScore >= 5.5 ? 'low' :
                            uiScore >= 4.0 ? 'poor' :
            ...
            // Rule 2 & 3: Critical (2.5-4.0) has two validation paths
                ...
                `).all(hs.id);
                            ...
                            if (hourPeriodScores.length > 0) {
                                const avgScore = (0, activityScoreCalculator_1.calculateTop80Average)(hourPeriodScores);
        ...
        const averageActivityScore = allScores.length > 0
            ? Math.round((0, activityScoreCalculator_1.calculateTop80Average)(allScores, `CurrentSession-${session.id}`) * 10) / 10 // Round to 1 decimal
            : 0;
    ...
    // Projects handler
    electron_1.ipcMain.handle('projects:fetch', async () => {
        if (!apiSyncService) {
            console.error('ApiSyncService not initialized');
            return [];
        }
    ...
    electron_1.ipcMain.handle('screenshots:get-by-date', async (_, date) => {
    ...
    electron_1.ipcMain.handle('screenshots:delete', async (_, screenshotIds) => {
        ...
        try {
            ...
            const cloudResults = await Promise.allSettled(screenshotIds.map(id => {
    ...
    electron_1.ipcMain.handle('screenshots:retry-sync', async (_, screenshotId) => {
    ...
    electron_1.ipcMain.handle('screenshots:fetch-signed-url', async (_, screenshotId) => {
    ...
    electron_1.ipcMain.handle('activity:get-periods-with-metrics', async (_, periodIds) => {
    ...
    electron_1.ipcMain.handle('dashboard:stats', async () => {
        try {
    ...
    electron_1.ipcMain.handle('productive-hours:get', async (_event, dateString) => {
        const currentUser = databaseService.getCurrentUser();
      ...
      `).all(currentUser.id, startOfDay.getTime(), endOfDay.getTime());
        ...
        `).all(screenshot.id);
            ...
            averageActivityScore = allScores.length > 0
                ? Math.round((0, activityScoreCalculator_1.calculateTop80Average)(allScores, 'LocalDaily') * 10) / 10 // Round to 1 decimal
                : 0;
        }
    ...
    electron_1.ipcMain.handle('weekly-marathon:get', async (_event, dateString) => {
        const currentUser = databaseService.getCurrentUser();
        if (!currentUser) {
            return null;
        }
        ...
        try {
            ...
            if (cloudData && cloudData.productiveHours !== undefined) {
                ...
                dailyData = cloudData.dailyData?.map((day, index) => {
      ...
      `).all(currentUser.id, startOfWeek.getTime(), endOfWeek.getTime());
        ...
        `).all(screenshot.id);
            ...
            // Calculate top 80% average, excluding lowest 20% of scores
            averageActivityScore = allScores.length > 0
                ? Math.round((0, activityScoreCalculator_1.calculateTop80Average)(allScores, 'LocalWeekly') * 10) / 10 // Round to 1 decimal
                : 0;
        }
    ...
    electron_1.ipcMain.handle('permissions:request', async (_, permissionId) => {
        return permissionsService.requestPermission(permissionId);
    });
    electron_1.ipcMain.handle('permissions:request-all', async () => {
        return permissionsService.requestAllPermissions();
    });
    // Debug handler for testing Sep 7 validation
    electron_1.ipcMain.handle('debug:test-sep7', async () => {
    ...
    electron_1.ipcMain.handle('debug:test-today', async () => {
        const currentUser = await databaseService.getCurrentUser();
        if (!currentUser) {
            throw new Error('No user found');
        }
        ...
        const lowActivityCount = screenshots.filter((s) => (s.metadata?.activityScore || 0) < 2.5).length;
    ...
    electron_1.ipcMain.handle('system:open-preferences', async (_, pane) => {
