...
class ActivityTracker extends events_1.EventEmitter {
    constructor(db) {
        ...
        this.memoryActivityPeriods = new Map();
        this.memoryScreenshots = new Map();
        this.windowCompletionTimer = null;
        this.currentWindowEnd = null;
        this.savedWindows = new Set();
        // Bot detection
        this.keyTimestamps = [];
  ...
  Screenshots in memory: ${this.memoryScreenshots.size}`);
    ...
    async saveCompletedWindow(windowEnd) {
        ...
        for (const [periodId, period] of this.memoryActivityPeriods) {
            ...
            const periodOverlapsWindow = (periodStartTime >= windowStartTime && periodStartTime < windowEndTime) || // Starts within window
    ...
    async saveMemoryPeriodsToDatabase(screenshotId, screenshotTime) {
    ...
    getMemoryActivityPeriods(windowStart, windowEnd) {
        const periods = [];
        for (const [_, period] of this.memoryActivityPeriods) {
            // Check if period falls within the window
            if ((period.periodStart >= windowStart && period.periodStart < windowEnd) ||
