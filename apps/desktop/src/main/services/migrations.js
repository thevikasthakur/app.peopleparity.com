...
class DatabaseMigrator {
    ...
    markMigrationAsApplied(version, name) {
        this.db.prepare('INSERT INTO migrations (version, name, applied_at) VALUES (?, ?, ?)').run(version, name, Date.now());
    }
    runMigrations() {
          ...
          `);
            ...
            },
            {
                ...
                up: (db) => {
                    ...
                    if (tableExists) {
                        const columns = db.prepare("PRAGMA table_info(activity_periods)").all();
                        const columnNames = columns.map(col => col.name);
