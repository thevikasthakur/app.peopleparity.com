...
const trackingMetadata_1 = require("./trackingMetadata");
const packageJson = __importStar(require("../../../package.json"));
class LocalDatabase {
    constructor() {
        // Store database in user's app data directory
        const userDataPath = electron_1.app.getPath('userData');
        this.dbPath = path_1.default.join(userDataPath, 'local_tracking.db');
        ...
        // Run migrations
        const migrator = new migrations_1.DatabaseMigrator(this.db);
    ...
    `);
        const insert = this.db.transaction((projects) => {
    ...
    async createSession(data) {
        ...
        const metadataService = trackingMetadata_1.TrackingMetadataService.getInstance();
    ...
    endActiveSessions(userId) {
        ...
        const activeSessions = this.db.prepare('SELECT id FROM sessions WHERE userId = ? AND isActive = 1').all(userId);
    ...
    getCurrentActivityPeriod(sessionId) {
        const stmt = this.db.prepare(`
    ...
    getActivityPeriodWithMetrics(periodId) {
    ...
    getActivityPeriodsWithMetrics(periodIds) {
    ...
    `);
        const periods = stmt.all(...periodIds);
    ...
    getRecentActivityPeriods(sessionId, limit = 10) {
        const stmt = this.db.prepare(`
    ...
    updateActivityPeriodScreenshot(periodId, screenshotId) {
        const stmt = this.db.prepare(`
    ...
    getActivityPeriodsForTimeRange(sessionId, windowStart, windowEnd) {
    ...
    `);
        ...
        const periods = stmt.all(sessionId, windowStartMs, windowEndMs, // for periodStart check
    ...
    createActivityPeriod(data) {
      ...
      LIMIT 1
    `).get(data.sessionId, normalizedStart.getTime() - 500, normalizedStart.getTime() + 500, normalizedEnd.getTime() - 500, normalizedEnd.getTime() + 500);
        if (existingPeriod) {
    ...
    `).get(data.localPath);
      ...
      `).get(existingScreenshot.id);
        ...
        const session = this.db.prepare('SELECT task FROM sessions WHERE id = ?').get(data.sessionId);
        const notes = session?.task || null;
        const screenshot = {
            id: data.id || crypto_1.default.randomUUID(), // Use provided ID or generate new one
            userId: data.userId,
            sessionId: data.sessionId,
    ...
    getScreenshotsByDate(userId, date) {
    ...
    `);
        const screenshots = stmt.all(userId, dateStart.getTime(), dateEnd.getTime());
        ...
        return screenshots.map((s) => {
            ...
            const scores = periods.map((p) => p.activityScore || 0);
            // Calculate weighted average using our new function
            const aggregatedScore = (0, activityScoreCalculator_1.calculateScreenshotScore)(scores);
    ...
    getTodayScreenshots(userId) {
      ...
      ORDER BY s.capturedAt ASC
    `);
        const screenshots = stmt.all(userId, startOfDay.getTime(), endOfDay.getTime());
    ...
    getScreenshotSyncStatus(screenshotId, periodIds) {
    ...
    `).get(screenshotId);
        ...
        let periodsSyncQuery = { totalPeriods: 0, syncedPeriods: 0, queuedPeriods: 0, maxAttempts: 0 };
      ...
      `).all(...periodIds);
        ...
        AND attempts < 5
      `).get(screenshotSyncQuery.queueId);
    ...
    updateScreenshotNotes(screenshotIds, notes) {
    ...
    `);
        let updatedCount = 0;
        const updateTransaction = this.db.transaction((ids) => {
            for (const id of ids) {
    ...
    deleteScreenshots(screenshotIds) {
        ...
        for (const id of screenshotIds) {
            const screenshot = checkStmt.get(id);
            if (screenshot) {
                ...
                if (screenshot.localPath)
                    filesToDelete.push(screenshot.localPath);
                if (screenshot.thumbnailPath)
                    filesToDelete.push(screenshot.thumbnailPath);
                // Find activity periods that reference this screenshot
                const periods = getPeriodsStmt.all(id);
                for (const period of periods) {
    ...
    getDateStats(userId, date) {
      ...
      ORDER BY s.capturedAt ASC
    `);
        const screenshots = screenshotsStmt.all(userId, startOfDay.getTime(), endOfDay.getTime());
      ...
      `);
            const periods = periodsStmt.all(screenshot.id);
            if (periods.length === 0) {
        ...
        for (let i = 0; i < screenshotScores.length; i++) {
            ...
            // Rule 2 & 3: Critical (2.5-4.0) has two possible validation paths
                ...
                WHERE screenshotId = ?
              `);
                            const periods = periodsStmt.all(hs.screenshot.id);
            ...
            console.log('[getDateStats] Screenshot', current.screenshot.id, 'weighted score:', current.uiScore.toFixed(1), 'valid:', isValid, validationReason ? `(${validationReason})` : '', 'mode:', current.screenshot.mode, 'prev:', prev ? prev.uiScore.toFixed(1) : 'none', 'next:', next ? next.uiScore.toFixed(1) : 'none');
        }
        console.log('[getDateStats] Valid screenshots:', validCount, 'Client minutes:', clientMinutes, 'Command minutes:', commandMinutes);
        return {
    ...
    saveCommandHourActivity(periodId, data) {
        const stmt = this.db.prepare(`
    ...
    `);
        ...
        stmt.run(id, periodId, data.uniqueKeys, data.productiveKeyHits, data.mouseClicks, data.mouseScrolls, data.mouseDistance, 0, Date.now());
        // Add to sync queue
        this.addToSyncQueue('command_activity', id, 'create', data);
    }
    saveClientHourActivity(periodId, data) {
        const stmt = this.db.prepare(`
    ...
    `);
        const id = crypto_1.default.randomUUID();
        stmt.run(id, periodId, data.codeCommitsCount || 0, data.filesSavedCount || 0, data.caretMovedCount || 0, data.textSelectionsCount || 0, data.filesOpenedCount || 0, data.tabsSwitchedCount || 0, data.netLinesCount || 0, data.copilotSuggestionsAccepted || 0, 0, Date.now());
        // Add to sync queue
    ...
    addToSyncQueue(entityType, entityId, operation, data) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(crypto_1.default.randomUUID(), entityType, entityId, operation, JSON.stringify(data), 0, null, Date.now());
    }
    ...
    markSynced(queueId) {
        ...
        const queueItem = this.db.prepare('SELECT entityType, entityId FROM sync_queue WHERE id = ?').get(queueId);
        if (queueItem) {
    ...
    saveRecentNote(userId, noteText) {
        console.log(`LocalDatabase: saveRecentNote called with userId: ${userId}, noteText: "${noteText}"`);
        const existing = this.db.prepare('SELECT * FROM recent_notes WHERE userId = ? AND noteText = ?').get(userId, noteText);
        if (existing) {
            console.log(`LocalDatabase: Note already exists, updating use count`);
            this.db.prepare('UPDATE recent_notes SET useCount = useCount + 1, lastUsedAt = ? WHERE id = ?').run(Date.now(), existing.id);
        }
        else {
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `);
            stmt.run(crypto_1.default.randomUUID(), userId, noteText, Date.now(), 1, Date.now());
        }
        ...
        if (activeSession) {
            ...
            const isSynced = this.db.prepare('SELECT isSynced FROM sessions WHERE id = ?').get(activeSession.id);
    ...
    getRecentNotes(userId, limit = 10) {
        const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
        const notes = stmt.all(userId, limit);
