...
class LocalDatabase {
    ...
    `);
        // Screenshots (uploaded to S3 via API)
        this.db.exec(`
    ...
    `);
        ...
        if (result.changes > 0) {
            // Add to sync queue
            const sessions = this.db.prepare('SELECT id FROM sessions WHERE userId = ? AND isActive = 0 AND endTime IS NOT NULL').all(userId);
    ...
    saveScreenshot(data) {
      ...
      INSERT INTO screenshots (
        id, userId, activityPeriodId, localPath, thumbnailPath, s3Url,
    ...
    `);
        stmt.run(screenshot);
        // Add to sync queue for S3 upload
    ...
    `);
        const screenshots = stmt.all(userId, todayStart.getTime());
        // Add the activity score from the period to each screenshot
        return screenshots.map(s => ({
            ...s,
            activityScore: s.periodActivityScore || 0
        }));
    }
