...
class MetricsCollector {
    constructor() {
        ...
        this.BOT_INTERVAL_THRESHOLD = 10; // ms - too fast to be human
        this.CONSISTENT_INTERVAL_THRESHOLD = 5; // ms - variance too low
        this.REPETITIVE_PATTERN_THRESHOLD = 10; // number of identical sequences
        this.initializeKeyCategories();
    }
    initializeKeyCategories() {
     ...
     */
    analyzeKeystrokePatterns(timestamps) {
     ...
     */
    analyzeMousePatterns(positions, clicks) {
        const reasons = [];
        let confidence = 0;
        ...
        if (positions.length > 10) {
            ...
            for (let i = 1; i < positions.length; i++) {
                const distance = Math.sqrt(Math.pow(positions[i].x - positions[i - 1].x, 2) +
            ...
            for (let i = 2; i < positions.length; i++) {
                const angle1 = Math.atan2(positions[i - 1].y - positions[i - 2].y, positions[i - 1].x - positions[i - 2].x);
                const angle2 = Math.atan2(positions[i].y - positions[i - 1].y, positions[i].x - positions[i - 1].x);
     ...
     */
    calculateDetailedScore(keyboardMetrics, mouseMetrics, botDetection, timeMetrics) {
        ...
        const weightedScore = components.keyHits * 0.25 + // 25% weight
        ...
        const hasHumanLikeTyping = keyHitsPerMin > 10 && keyHitsPerMin < 200 && // Reasonable typing speed (10-200 keys/min)
     ...
     */
    classifyActivity(score, metrics) {
        const tags = [];
        // Add relevant tags
        ...
        let confidence = 0.5;
        if (metrics.keyboard.totalKeystrokes > 50)
            confidence += 0.2;
        if (metrics.mouse.totalClicks > 10)
            confidence += 0.15;
        if (metrics.timeMetrics.periodDurationSeconds >= 60)
            confidence += 0.15;
        return {
            category,
     ...
     */
    generateMetricsBreakdown(rawMetrics, periodDuration) {
        ...
        const scoreCalculation = this.calculateDetailedScore(keyboardMetrics, mouseMetrics, botDetection, timeMetrics);
        const classification = this.classifyActivity(scoreCalculation.finalScore, {
    ...
    // Helper methods
    calculateAvgInterval(timestamps) {
    ...
    calculateStdDeviation(timestamps) {
        if (timestamps.length < 2)
            return 0;
        const intervals = [];
        for (let i = 1; i < timestamps.length; i++) {
    ...
    getLastIntervals(timestamps, limit) {
        const intervals = [];
    ...
    calculateAvgMouseSpeed() {
        ...
        for (let i = 1; i < this.mousePositions.length; i++) {
            const distance = Math.sqrt(Math.pow(this.mousePositions[i].x - this.mousePositions[i - 1].x, 2) +
    ...
    recordKeystroke(keycode, timestamp) {
    ...
    recordMousePosition(x, y, timestamp) {
     ...
     */
    calculateSimpleScore(keystrokes, uniqueKeystrokes, mouseClicks, mouseScrolls, mouseDistance, periodDuration) {
     ...
     */
    detectSuspiciousPatterns(keystrokes, uniqueKeystrokes, mouseClicks) {
        let penaltyScore = 0;
