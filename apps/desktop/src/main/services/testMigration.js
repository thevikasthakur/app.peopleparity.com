...
function createOldSchemaDatabase(dbPath) {
...
function verifyMigration(db) {
    ...
    const activityColumns = db.prepare("PRAGMA table_info(activity_periods)").all();
...
// Main test execution
function runTest() {
    const testDbPath = path_1.default.join(process.cwd(), 'test_migration.db');
    try {
        // Step 1: Create database with old schema
        const db = createOldSchemaDatabase(testDbPath);
        // Step 2: Run migrations
        console.log('\n🚀 Running migrations...');
        const migrator = new migrations_1.DatabaseMigrator(db);
