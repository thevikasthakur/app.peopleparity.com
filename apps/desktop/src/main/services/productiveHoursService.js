...
const managerMessages_1 = require("../utils/managerMessages");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class ProductiveHoursService {
    constructor(db) {
        this.db = db;
        this.holidayConfig = {};
     ...
     */
    async calculateProductiveHours(userId, date = new Date()) {
     ...
     */
    getHustleMessage(hours, markers, lastActivityScore = 5) {
        const now = new Date();
        const context = {
        ...
        return (0, managerMessages_1.getManagerMessage)(context);
    }
    /**
     * Get attendance status based on productive hours
     */
    getAttendanceStatus(hours, markers, date = new Date()) {
     ...
     */
    async calculateWeeklyHours(userId, date = new Date()) {
     ...
     */
    async getDailyHoursForWeek(userId, date) {
        const db = this.db;
     ...
     */
    getDayAttendanceStatus(hours, isHolidayWeek, weekTotal = 0, isFuture = false, isCurrentDay = false, isWeekend = false) {
     ...
     */
    calculateWeeklyAttendance(hours, markers, dailyData, selectedDate) {
        ...
        if (dailyData && dailyData.length === 7) {
            ...
            dailyStatuses = dailyData.map((data, index) => {
                ...
                const dayStatus = this.getDayAttendanceStatus(data.hours, markers.hasHoliday, hours, data.isFuture, isCurrentDay, isWeekend);
                return {
                    day: dayLabels[index],
     ...
     */
    getWeeklyMessage(hours, attendance, markers) {
        const now = new Date();
        ...
        return (0, managerMessages_1.getWeeklyMarathonMessage)(context);
    }
}
exports.ProductiveHoursService = ProductiveHoursService;
//# sourceMappingURL=productiveHoursService.js.map