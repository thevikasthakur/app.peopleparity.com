...
export class SpikeDetector {
  ...
  private readonly maxHistorySize = 20; // Keep last 20 periods (about 20 minutes)
  private readonly spikeThreshold = 3.0; // Standard deviations for spike detection
  private readonly minHistoryForDetection = 3; // Need at least 3 periods to detect spikes
   ...
   */
  private detectSpikes(currentData: ActivityData): SpikeDetectionResult {
   ...
   */
  private detectPatternAnomalies(current: ActivityData, history: ActivityData[]): {
    isAnomalous: boolean;
    score: number;
    reason: string;
  } {
...
let globalSpikeDetector: SpikeDetector | null = null;

export function getGlobalSpikeDetector(): SpikeDetector {
...
export function resetGlobalSpikeDetector() {
  if (globalSpikeDetector) {
