  ...
  async getDailyProductiveHours(userId: string, date: Date) {
    try {
      ...
      if (screenshots.length > 0) {
        ...
        console.log(
          `Found ${activityPeriods.length} activity periods for these screenshots`
        );

        // Map activity periods to screenshots
        for (const screenshot of screenshots) {
          screenshot.activityPeriods = activityPeriods.filter(
            (ap) => ap.screenshotId === screenshot.id
          );
        }
      }
      ...
      for (let i = 0; i < screenshotScores.length; i++) {
        ...
        const next =
          i < screenshotScores.length - 1 ? screenshotScores[i + 1] : null;
        ...
        if (current.weightedScore >= 40) {
          isValid = true;
          console.log(
            `Screenshot ${i}: Score ${current.uiScore.toFixed(
              1
            )} >= 4.0 -> VALID`
          );
        }
        // Rule 2 & 3: Critical (2.5-4.0) has two possible validation paths
          ...
          if (
            ...
            console.log(
              `Screenshot ${i}: Critical score ${current.uiScore.toFixed(
                1
              )} with neighbor >= 4.0 (${neighborInfo}) -> VALID`
            );
          }
          // Rule 3: Check hourly average condition
            ...
            if (hourScreenshots.length >= 6) {
              ...
              for (const hs of hourScreenshots) {
                const periodScores =
                  hs.screenshot.activityPeriods?.map(
                    (p) => p.activityScore || 0
                  ) || [];
              ...
              if (hourPeriodScores.length > 0) {
                const avgScore = this.calculateTop80Average(
                  hourPeriodScores.map((s) => s / 10)
                );
                ...
                if (avgScore >= 4.0) {
                  ...
                  console.log(
                    `Screenshot ${i}: Critical score ${current.uiScore.toFixed(
                      1
                    )} with hourly avg ${avgScore.toFixed(1)} >= 4.0 (${
                      hourScreenshots.length
                    } screenshots in hour) -> VALID`
                  );
                } else {
                  console.log(
                    `Screenshot ${i}: Critical score ${current.uiScore.toFixed(
                      1
                    )} with hourly avg ${avgScore.toFixed(1)} < 4.0 -> INVALID`
                  );
                }
              }
            } else {
              console.log(
                `Screenshot ${i}: Critical score ${current.uiScore.toFixed(
                  ...
                  hourScreenshots.length
                } screenshots in hour (< 6 required) -> INVALID`
              );
            }
          }
        }
  ...
  async getWeeklyProductiveHours(userId: string, date: Date) {
    try {
      ...
      const screenshots = await this.screenshotRepository
        ...
        .where("screenshot.userId = :userId", { userId })
        .andWhere(
          `screenshot."capturedAt"::timestamptz >= :startOfWeek::timestamptz`,
          { startOfWeek: startOfWeek.toISOString() }
        )
        .andWhere(
          `screenshot."capturedAt"::timestamptz <= :endOfWeek::timestamptz`,
          { endOfWeek: endOfWeek.toISOString() }
        )
        .orderBy("screenshot.capturedAt", "ASC")
        .getMany();
