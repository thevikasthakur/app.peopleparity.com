...
function Settings() {
    const { user, refreshUser } = (0, AuthContext_1.useAuth)();
    ...
    (0, react_1.useEffect)(() => {
        ...
        const loadSettings = async () => {
            try {
                const userData = await apiService_1.apiService.getUserSettings();
    ...
    const handleSave = async () => {
        ...
        setSuccessMessage('');
        try {
            await apiService_1.apiService.updateUserSettings({ timezone });
            await refreshUser();
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center">
              <lucide_react_1.Clock className="w-6 h-6 text-white"/>
            </div>
            <div>
          ...
          {successMessage && (<div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm">
              {successMessage}
            </div>)}

          {isLoading ? (<div className="flex items-center justify-center py-12">
              <lucide_react_1.Loader className="w-8 h-8 animate-spin text-indigo-600"/>
              ...
              <div>
                ...
                <select value={timezone} onChange={(e) => setTimezone(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
              ...
              <div className="pt-6 border-t border-gray-200">
                <button onClick={handleSave} disabled={isSaving} className="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                  {isSaving ? (<>
