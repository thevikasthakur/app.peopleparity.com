...
exports.ScreenshotGrid = ScreenshotGrid;
const react_1 = require("react");
const react_dom_1 = __importDefault(require("react-dom"));
const framer_motion_1 = require("framer-motion");
const lucide_react_1 = require("lucide-react");
const apiService_1 = require("../services/apiService");
function percentageToTenScale(percentage) {
...
function ScreenshotGrid({ screenshots, isLoading, userRole, userTimezone, developerTimezone, isViewingAsAdmin }) {
    const [selectedScreenshot, setSelectedScreenshot] = (0, react_1.useState)(null);
    const [selectedScreenshotIndex, setSelectedScreenshotIndex] = (0, react_1.useState)(-1);
    const [selectedIds, setSelectedIds] = (0, react_1.useState)(new Set());
    const [hoveredId, setHoveredId] = (0, react_1.useState)(null);
    const [signedFullUrl, setSignedFullUrl] = (0, react_1.useState)(null);
    const [loadingSignedUrl, setLoadingSignedUrl] = (0, react_1.useState)(false);
    const [activityPeriods, setActivityPeriods] = (0, react_1.useState)([]);
    const [loadingActivityPeriods, setLoadingActivityPeriods] = (0, react_1.useState)(false);
    const [expandedMinutes, setExpandedMinutes] = (0, react_1.useState)(new Set());
    const signedUrlCacheRef = (0, react_1.useRef)(new Map());
    const getTimestamp = (screenshot) => {
    ...
    const getActivityName = (screenshot) => {
    ...
    (0, react_1.useEffect)(() => {
        const fetchSignedUrl = async () => {
            ...
            try {
                const response = await apiService_1.apiService.getSignedUrl(selectedScreenshot.id);
    ...
    (0, react_1.useEffect)(() => {
        const fetchActivityPeriods = async () => {
            ...
            try {
                const details = await apiService_1.apiService.getScreenshotDetails(selectedScreenshot.id);
    ...
    if (!screenshots || screenshots.length === 0) {
        return (<div className="flex flex-col items-center justify-center h-64 glass-card">
        <lucide_react_1.Monitor className="w-12 h-12 text-gray-400 mb-4"/>
    ...
    return (<div className="space-y-4">
      {Object.entries(hourGroups).sort((a, b) => a[0].localeCompare(b[0])).map(([hourKey, hourScreenshots]) => {
            ...
            const validScreenshots = hourScreenshots.filter(s => s !== null);
            ...
            return (<div key={hourKey} className="space-y-2">
            {showDateSeparator && (<div className="flex items-center gap-3 py-2 mt-4 first:mt-0">
                <div className="flex-1 h-px bg-gradient-to-r from-transparent via-gray-300 to-transparent"/>
                <div className="flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-full">
                  <lucide_react_1.Calendar className="w-4 h-4 text-gray-500"/>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              {isViewingAsAdmin && developerTimezone && developerTimezone !== userTimezone ? (<div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-6 gap-2">
              {hourScreenshots.map((screenshot, index) => {
                    ...
                    if (!screenshot) {
                        return (<div key={`empty-${hour}-${index}`} className="relative aspect-video rounded-lg bg-gray-100 border-2 border-dashed border-gray-200">
                    ...
                    return (<framer_motion_1.motion.div key={screenshot.id} whileHover={{ scale: 1.05 }} className={`
                    ...
                    `} onClick={() => {
                        ...
                        }} onMouseEnter={() => setHoveredId(screenshot.id)} onMouseLeave={() => setHoveredId(null)}>
                    <img src={getThumbnailUrl(screenshot)} alt={`Screenshot at ${new Date(getTimestamp(screenshot)).toLocaleTimeString()}`} className={`w-full h-full object-cover transition-opacity duration-200 ${isHovered
                            ? 'opacity-100'
                            : scoreOutOf10 < 2.5
                                ? 'opacity-20'
                                : scoreOutOf10 < 4.0
                                    ? 'opacity-75'
                                    : 'opacity-100'}`} loading="lazy"/>
                    ...
                    <button className={`
                      ...
                      {isSelected && <lucide_react_1.Check className="w-4 h-4"/>}
                    </button>

                    <div className="absolute top-2 right-2">
                      <div className="text-right">
                        <div className="px-2 py-1 rounded-full text-xs font-medium backdrop-blur text-white" style={{ backgroundColor: level.color + 'CC' }}>
                          {scoreOutOf10.toFixed(1)}
                        </div>
                        <div className="mt-1 px-1 py-0.5 rounded text-[10px] font-medium backdrop-blur text-white" style={{ backgroundColor: level.color + 'AA' }}>
                          {level.name}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="absolute bottom-2 left-2 flex flex-col gap-1">
                      <div className={`px-[2px] py-0.5 rounded text-[9px] font-medium backdrop-blur text-white ${(screenshot.mode === 'client' || screenshot.mode === 'client_hours')
                            ? 'bg-indigo-500/80'
                            : 'bg-emerald-500/80'}`} style={{ width: '27px' }}>
                      ...
                      <div className="px-2 py-1 rounded text-xs font-semibold backdrop-blur text-white bg-black/70">
                        {isViewingAsAdmin && developerTimezone && developerTimezone !== userTimezone ? (<div className="flex flex-col gap-0.5">
            ...
            {activityGroups.length > 0 && (<div className="relative h-6 bg-gray-100 rounded overflow-hidden">
                {activityGroups.map((group, idx) => {
                        const leftPosition = (group.startIdx / 6) * 100;
                        const width = (group.count / 6) * 100;
                        return (<div key={idx} className="absolute h-full flex items-center justify-center text-[11px] font-medium text-white px-1 rounded-sm" style={{
                                left: `${leftPosition}%`,
                ...
                <div className="absolute right-0 top-0 h-full px-2 flex items-center text-[10px] font-bold text-white" style={{
      ...
      {selectedScreenshot && react_dom_1.default.createPortal(<framer_motion_1.AnimatePresence>
          <framer_motion_1.motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => {
                setSelectedScreenshot(null);
                setSelectedScreenshotIndex(-1);
            }}>
            <framer_motion_1.motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full h-full bg-white overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b">
                ...
                : 'bg-emerald-100 text-emerald-700'}
                  ...
                  {isViewingAsAdmin && developerTimezone && developerTimezone !== userTimezone ? (<div className="flex flex-col text-sm">
                  ...
                  {getActivityName(selectedScreenshot) && (<span className="text-sm text-gray-700 ml-2">
              ...
              <div className="h-[calc(100%-4rem)] flex">
                <div className="flex-1 bg-gray-100 flex items-center justify-center overflow-auto relative">
                  <button onClick={() => navigateScreenshot('prev')} className="absolute left-6 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-full shadow-lg transition-all hover:scale-110 z-10" title="Previous screenshot (←)">
                    <lucide_react_1.ChevronLeft className="w-6 h-6 text-white"/>
                  </button>

                  <button onClick={() => navigateScreenshot('next')} className="absolute right-6 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-full shadow-lg transition-all hover:scale-110 z-10" title="Next screenshot (→)">
                    <lucide_react_1.ChevronRight className="w-6 h-6 text-white"/>
                  </button>
                  ...
                  {loadingSignedUrl ? (<div className="flex items-center justify-center gap-2 text-gray-400">
                      <lucide_react_1.Loader className="w-8 h-8 animate-spin"/>
                      <span>Loading image...</span>
                    </div>) : signedFullUrl ? (<img src={signedFullUrl} alt="Full size screenshot" className="w-full h-full object-contain"/>) : (<div className="flex items-center justify-center text-gray-400">
                ...
                return (<>
                        ...
                        backgroundColor: level.color
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs px-2 py-1 rounded-full text-white font-medium" style={{ backgroundColor: level.color }}>
                                    {level.name}
                                  </span>
                                </div>
                              </>);
                      ...
                      {selectedScreenshot.userName && (<div>
                      ...
                      {selectedScreenshot.deviceInfo && (<div>
                      ...
                      {(userRole === 'super_admin' || userRole === 'org_admin') && (<div className="pt-2 border-t">
                  ...
                  <div className="p-4">
                    ...
                    {loadingActivityPeriods ? (<div className="text-center py-4">
                        <lucide_react_1.Loader className="w-6 h-6 animate-spin mx-auto text-gray-400"/>
                        <p className="text-xs text-gray-500 mt-2">Loading metrics...</p>
                      </div>) : activityPeriods.length > 0 ? (<div className="space-y-2">
                    ...
                    const periodEndTimeAdmin = isViewingAsAdmin && developerTimezone && developerTimezone !== userTimezone
                        ...
                        : null;
                    return (<div key={period.id} className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  {isViewingAsAdmin && developerTimezone && developerTimezone !== userTimezone && periodEndTimeAdmin ? (<div className="flex flex-col">
                                    ...
                                    </div>) : (<span className="text-sm font-medium">{periodEndTime}</span>)}
                                  <button onClick={() => {
                            ...
                            setExpandedMinutes(newExpanded);
                                    ...
                                    {expandedMinutes.has(index) ? (<lucide_react_1.ChevronUp className="w-4 h-4 text-gray-500"/>) : (<lucide_react_1.ChevronDown className="w-4 h-4 text-gray-500"/>)}
                                  </button>
                                </div>
                                <div className="text-center">
                                  <div className="text-sm font-semibold px-3 py-1 rounded text-white inline-block" style={{ backgroundColor: level.color }}>
                                    {scoreOutOf10.toFixed(1)}
                                  </div>
                              ...
                              <framer_motion_1.AnimatePresence>
                                {expandedMinutes.has(index) && period.metrics && (<framer_motion_1.motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                                    <div className="mt-3 pt-3 border-t space-y-3">
                                      {/* Keyboard Activity */}
                                      {period.metrics.keyboard && (<div className="flex items-start gap-2">
                                          <lucide_react_1.Keyboard className="w-4 h-4 text-gray-500 mt-0.5"/>
                                          <div className="flex-1 text-xs">
                                            ...
                                            {period.metrics.keyboard.typingRhythm && (<div className="mt-1 pt-1 border-t">
                                                <span className={`text-[10px] ${period.metrics.keyboard.typingRhythm.consistent
                                        ? 'text-green-600'
                                        : 'text-yellow-600'}`}>
                                      ...
                                      {/* Mouse Activity */}
                                      {period.metrics.mouse && (<div className="flex items-start gap-2">
                                          <lucide_react_1.MousePointer className="w-4 h-4 text-gray-500 mt-0.5"/>
                                          <div className="flex-1 text-xs">
                                            <div className="flex justify-between mb-1">
                                            ...
                                            {period.metrics.mouse.movementPattern && (<div className="mt-1 pt-1 border-t">
                                                <span className={`text-[10px] ${period.metrics.mouse.movementPattern.smooth
                                        ? 'text-green-600'
                                        : 'text-yellow-600'}`}>
                                      ...
                                      {period.metrics.botDetection && (period.metrics.botDetection.keyboardBotDetected || period.metrics.botDetection.mouseBotDetected) && (<div className="bg-orange-50 rounded p-2 border border-orange-200">
                                          <div className="flex items-center gap-2 mb-2">
                                            <lucide_react_1.AlertCircle className="w-4 h-4 text-orange-500"/>
...
//# sourceMappingURL=ScreenshotGrid.js.map