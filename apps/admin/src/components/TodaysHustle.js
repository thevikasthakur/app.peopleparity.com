...
const getActivityMessage = (score, hours, isWeekend) => {
    if (isWeekend) {
        return "Weekend warrior mode activated! Every hour counts toward your weekly goals.";
    }
    if (hours < 2) {
        return "Just getting started! Time to build momentum.";
    }
    if (score >= 8.5) {
        return "Outstanding productivity! You're in the zone today.";
    }
    else if (score >= 7.0) {
        return "Good work! Maintaining steady progress throughout the day.";
    }
    else if (score >= 5.5) {
        return "Keep pushing! You can improve your activity score with focused work.";
    }
    else {
        return "Stay focused and minimize distractions for better tracking.";
    }
};
const TodaysHustle = ({ selectedDate, isToday, userId }) => {
    const [hustleData, setHustleData] = (0, react_1.useState)(null);
    ...
    }, [selectedDate, isToday, userId]);
    const loadHustleData = async () => {
        try {
            // Fetch productive hours data from API
            const response = await apiService_1.apiService.getProductiveHours({
                userId,
                date: selectedDate.toISOString().split('T')[0],
            });
            setHustleData(response);
        }
        catch (error) {
    ...
    const getMilestoneStatus = () => {
        if (productiveHours > markers.fullAttendance)
            return 'extra';
        if (productiveHours >= markers.fullAttendance && productiveHours <= markers.fullAttendance)
            return 'full';
        if (productiveHours >= markers.threeQuarterAttendance && productiveHours < markers.fullAttendance)
            return 'three-quarter';
        if (productiveHours >= markers.halfAttendance && productiveHours < markers.threeQuarterAttendance)
            return 'half';
        return 'none';
    };
    ...
    return (<div className="glass-card p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          <div className="p-2 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 rounded-lg">
            <lucide_react_1.TrendingUp className="w-5 h-5 text-indigo-600"/>
          </div>
          <div>
          ...
          {markers.isHolidayWeek && (<span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full animate-pulse">
              🎉 Holiday Mode
            </span>)}
        </div>
      ...
      {!attendance.isWeekend ? (<div className="grid grid-cols-5 gap-1.5 mb-4">
          {/* No Attendance */}
          <div className={`p-2 rounded-lg border transition-all ${milestoneStatus === 'none'
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              {milestoneStatus === 'none' ? (<lucide_react_1.AlertCircle className="w-3 h-3 text-red-500"/>) : (<lucide_react_1.AlertCircle className="w-3 h-3 text-gray-300"/>)}
            </div>
            <p className="text-sm font-bold text-gray-900">&lt;{markers.halfAttendance.toFixed(1)}h</p>
            <p className="text-[10px] text-gray-500">0% attendance</p>
          </div>

          {/* Half Day */}
          <div className={`p-2 rounded-lg border transition-all ${milestoneStatus === 'half'
                ? 'bg-amber-50 border-amber-300 ring-2 ring-amber-300'
                : 'bg-gray-50 border-gray-200'}`}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-medium text-gray-600">Half Day</span>
              {milestoneStatus === 'half' ? (<lucide_react_1.CheckCircle className="w-3 h-3 text-amber-500"/>) : (<lucide_react_1.AlertCircle className="w-3 h-3 text-gray-300"/>)}
            </div>
            <p className="text-sm font-bold text-gray-900">{markers.halfAttendance.toFixed(1)}h</p>
          ...
          {/* Good Day */}
          <div className={`p-2 rounded-lg border transition-all ${milestoneStatus === 'three-quarter'
                ? 'bg-blue-50 border-blue-300 ring-2 ring-blue-300'
                : 'bg-gray-50 border-gray-200'}`}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-medium text-gray-600">Good Day</span>
              {milestoneStatus === 'three-quarter' ? (<lucide_react_1.CheckCircle className="w-3 h-3 text-blue-500"/>) : (<lucide_react_1.Target className="w-3 h-3 text-gray-300"/>)}
            </div>
            <p className="text-sm font-bold text-gray-900">{markers.threeQuarterAttendance.toFixed(1)}h</p>
            <p className="text-[10px] text-gray-500">75% attendance</p>
          </div>

          {/* Full Day */}
          <div className={`p-2 rounded-lg border transition-all ${milestoneStatus === 'full'
                ? 'bg-green-50 border-green-300 ring-2 ring-green-300'
                : 'bg-gray-50 border-gray-200'}`}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-medium text-gray-600">Full Day</span>
              {milestoneStatus === 'full' ? (<lucide_react_1.Award className="w-3 h-3 text-green-500"/>) : (<lucide_react_1.Flag className="w-3 h-3 text-gray-300"/>)}
            </div>
            <p className="text-sm font-bold text-gray-900">{markers.fullAttendance.toFixed(1)}h</p>
            <p className="text-[10px] text-gray-500">100% attendance</p>
          </div>

          {/* Extra Mileage */}
          <div className={`p-2 rounded-lg border transition-all ${milestoneStatus === 'extra'
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              {milestoneStatus === 'extra' ? (<lucide_react_1.Zap className="w-3 h-3 text-purple-500"/>) : (<lucide_react_1.Zap className="w-3 h-3 text-gray-300"/>)}
            </div>
            <p className="text-sm font-bold text-gray-900">&gt;{markers.fullAttendance.toFixed(1)}h</p>
          ...
          <div className="flex items-center justify-center space-x-3">
            ...
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-2 mt-2">
                <lucide_react_1.Trophy className="w-4 h-4 text-purple-500"/>
      ...
      <div className="relative mb-5">
        ...
        <div className="h-10 bg-gray-100 rounded-xl overflow-hidden relative shadow-inner">
          {/* Progress Fill */}
          <div className="h-full transition-all duration-700 ease-out relative rounded-xl" style={{
            width: `${percentage}%`,
            background: getProgressGradient(),
            boxShadow: '0 2px 8px rgba(0,0,0,0.15)'
        }}>
            {/* Animated shimmer for overachievement */}
            {productiveHours > markers.fullAttendance && (<div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent animate-shimmer"></div>)}

            {/* Current value indicator */}
            <div className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur px-2 py-1 rounded text-xs font-bold shadow-sm" style={{ color: attendance.color }}>
              {formatHoursToHM(productiveHours)}
            </div>
          </div>
          ...
          <div className="absolute inset-0 pointer-events-none">
            {/* Half Day Marker */}
            <div className="absolute h-full flex flex-col justify-end pb-1" style={{ left: `${halfMarkerPos}%` }}>
            ...
            {/* Good Day Marker */}
            <div className="absolute h-full flex flex-col justify-end pb-1" style={{ left: `${threeQuarterMarkerPos}%` }}>
            ...
            {/* Full Day Marker */}
            <div className="absolute h-full flex flex-col justify-end pb-1" style={{ left: `${fullMarkerPos}%` }}>
        ...
        <div className="flex justify-between mt-8 text-xs text-gray-500">
          ...
          <span className="font-medium">{markers.maxScale.toFixed(1)}h target</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: attendance.color }}/>
            <span className="text-xs text-gray-600">
              {attendance.isWeekend ? "Weekend" : (isToday ? "Today's" : "Day's")} {attendance.isWeekend ? "Hours" : "Achievement"}:
            </span>
            <span className="font-bold text-sm px-2 py-1 rounded-lg" style={{
            backgroundColor: `${attendance.color}15`,
            color: attendance.color,
            border: `1px solid ${attendance.color}30`
        }}>
            ...
            {attendance.isWeekend && (<span className="text-xs text-purple-600 ml-2">
                (No attendance on weekends, but hours count towards weekly total)
              </span>)}
          </div>

          {productiveHours > markers.fullAttendance && !attendance.isWeekend && (<div className="flex items-center gap-1 animate-bounce">
        ...
        <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4 border border-gray-200">
          ...
          <div className="border-t border-gray-300 pt-2">
            <p className="text-xs text-gray-600 italic leading-relaxed">
              {getActivityMessage(averageActivityScore, productiveHours, attendance.isWeekend || false)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 pt-3 border-t border-gray-200 text-xs text-gray-500">
        <div className="flex items-center justify-between">
          ...
          {markers.isHolidayWeek && (<p className="text-amber-600 font-medium">
...
exports.TodaysHustle = TodaysHustle;
exports.default = exports.TodaysHustle;
//# sourceMappingURL=TodaysHustle.js.map