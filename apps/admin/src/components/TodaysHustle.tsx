...
const getActivityMessage = (score: number, hours: number, isWeekend: boolean): string => {
  if (isWeekend) {
...
export const TodaysHustle: React.FC<TodaysHustleProps> = ({ selectedDate, isToday, userId }) => {
  ...
  const loadHustleData = async () => {
    try {
      ...
      const response = await apiService.getProductiveHours({
        userId,
